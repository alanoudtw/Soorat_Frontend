...
class SooratSupabaseUser extends BaseAuthUser {
  SooratSupabaseUser(this.user);
  User? user;
...
Stream<BaseAuthUser> sooratSupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = SooratSupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
